 ...
 */
export async function setFormState(formElementId: string, name: string, value: string) {
// update locally stored form state when each field is updated and update expiration to 24 hours.
const currentStorage = await JSON.parse(sessionStorage.getItem(formElementId)) || {};
if(value.length) currentStorage[name] = value;
else {
  if(currentStorage[name]) delete currentStorage[name]
}
if(Object.keys(currentStorage).length) sessionStorage.setItem(formElementId, JSON.stringify(currentStorage))
else clearFormState(formElementId)
};

export function clearFormState(formElementId: string) {
  sessionStorage.removeItem(formElementId)
};

 export async function getFormState(formElementId: string) {
